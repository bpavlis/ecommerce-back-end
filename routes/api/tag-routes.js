...
router.get('/', async (req, res) => {
  ...
  try { 
    const payload = await Tag.findAll({
      include: [{model: Product}, {model: ProductTag, through: ProductTag, as: 'tag_product'}]
    });
...
router.get('/:id', async (req, res) => {
  ...
  try { 
    const payload = await Tag.findByPk(req.params.id, {
      include: [{ model: Category}, { model: Product, attributes: ['tag_name'], through: ProductTag, as: 'tag_product'}]
    });
    res.status(200).json({status: "success", payload })
  } catch (err) {
    res.status(500).json({status: "error", payload: err.message })
